    ...
    for (let i = 0; i < NB_ROUNDE_20222023; i++) {
        ...
        for (let j = 0; j < EQUIPES_20222023.length; j++) {
            ...
            if ((i + 1) % 2 === 0) {
                ...
                let joueur = tr.appendChild(document.createElement("td"))
                joueur.id = 'Draft2022choix' + choixTotal
                joueur.setAttribute('class', 'nomChoix')
                joueur.innerHTML = ordreRepechage[EQUIPES_20222023[EQUIPES_20222023.length - 1 - j]][i];
            } else {
                tr.appendChild(document.createElement("td")).innerHTML = EQUIPES_20222023[j]
                let joueur = tr.appendChild(document.createElement("td"))
                joueur.id = 'Draft2022choix' + choixTotal
                joueur.setAttribute('class', 'nomChoix')
                joueur.innerHTML = ordreRepechage[EQUIPES_20222023[j]][i];
            }
            choixTotal++;
        }
    }
