...
function liveFeed() {
	...
	rechercheJoueur.addEventListener("keypress", function (e) {
		if (e.key === "Enter") {
			let id = getActivePlayerId(rechercheJoueur.value);
			if (id) {
				genererPageJoueur(id);
			} else {
	...
	document
		...
		.addEventListener("click", function (e) {
			...
			for (let i = 0; i < tab.length; i++) {
				...
				document.getElementById("mobile-menu-ul").className =
					"container-aff-oui";
			}
		});
...
function getActivePlayerId(name) {
	...
	let response = JSON.parse(player.response);
	let id = false;
...
function genererPageJoueur(id) {
	...
	if (!isNaN(parseInt(id))) {
		...
		name.send();
		let codePosition = JSON.parse(name.response).people[0].primaryPosition
			.abbreviation;
		content.innerHTML = "";
		...
		if (codePosition !== "G") {
			...
			for (let i = 0; i < arraySeasons.length; i++) {
				...
				if (ok) {
					...
					season.innerHTML =
						arraySeasons[i].season.slice(0, 4) +
						"-" +
						arraySeasons[i].season.slice(4);
					...
					let ppgStat = 0;
					if (pjStat >= 1) ppgStat = (ptsStat / pjStat).toFixed(2);
			...
			let cPpg = 0;
			if (cPj >= 1) cPpg = (cPts / cPj).toFixed(2);
		...
		abbCall.onload = () => {
			...
			picture.src =
				...
				".png";
			picture.alt =
				"Photo indisponible pour les joueurs pas dans la AHL et NHL";
			...
			if (JSON.parse(name.response).people[0].shootsCatches === "R")
				rightVsLeft = "Right";
			positions.innerHTML =
				JSON.parse(name.response).people[0].primaryPosition.name +
				" | " +
				"Shoots: " +
				rightVsLeft;
	...
	schedule.onload = () => {
		...
		if (matchs.length > 0) {
			for (let i = 0; i < matchs.length; i++) {
				...
				let time = table.firstElementChild.appendChild(
					document.createElement("th")
				);
				let away = tableChildren
					.item(1)
					.appendChild(document.createElement("td"));
				let awayScore = tableChildren
					.item(1)
					.appendChild(document.createElement("td"));

				let home = tableChildren
					.item(2)
					.appendChild(document.createElement("td"));
				let homeScore = tableChildren
					.item(2)
					.appendChild(document.createElement("td"));
				...
				liveFeed.open(
					"GET",
					"https://statsapi.web.nhl.com" + matchs[i].link,
					false
				);
				liveFeed.send();
				...
				awayImg.src =
					"https://assets.nhle.com/logos/nhl/svg/" + awayAbb + "_dark.svg";
				homeImg.src =
					"https://assets.nhle.com/logos/nhl/svg/" + homeAbb + "_dark.svg";
				...
				if (game.gameData.status.abstractGameState !== "Preview")
					time.innerHTML =
						game.liveData.linescore.currentPeriodOrdinal +
						"<br>" +
